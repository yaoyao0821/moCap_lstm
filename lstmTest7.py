...
class LSTMRNN(object):
    ...
    def add_cell(self):
        ...
        lstm_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_cell, input_keep_prob=1.0, output_keep_prob=0.6)
...
if __name__ == '__main__':
    ...
    saver = tf.train.Saver()
    # saver = tf.train.import_meta_graph('./model/test2.meta')

    # saver.restore(sess, tf.train.latest_checkpoint('./model/'))
    saver.restore(sess, './model/test2')
    # graph = tf.get_default_graph()
    graph = tf.get_default_graph()

    model.pred = tf.get_collection('predict')[0]
    model.xs = graph.get_operation_by_name("xs").outputs[0]
    ...
    for i in range(30000):
        ...
        #
        if test < 1500 and test < min_cost:
            saver = tf.train.Saver()
            model_path = "./model/test3"
