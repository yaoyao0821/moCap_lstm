...
if __name__ == '__main__':
    ...
    for i in range(1000):
        seq, res, xs = get_batch()
        if i == 0:
            seq = np.concatenate((seq, res), axis=2)

            print(i, seq.shape, res.shape, xs.shape)
            feed_dict = {
                    model.xs: seq,
                    model.ys: res,
            ...
            print(i, seq.shape, res.shape, xs.shape,pred.shape)
            seq = np.concatenate((seq, pred), axis=2)
