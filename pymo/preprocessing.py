...
#        return X
    ...
    def _to_quat(self, X):
        ...
        for track in X:
            ...
            # Create a new DataFrame to store the exponential map rep
            quat_df = pd.DataFrame(index=euler_df.index)
            ...
            quat_df[rxp] = pd.Series(data=euler_df[rxp], index=quat_df.index)
            quat_df[ryp] = pd.Series(data=euler_df[ryp], index=quat_df.index)
            quat_df[rzp] = pd.Series(data=euler_df[rzp], index=quat_df.index)
            ...
            for joint in joints:
                ...
                # euler = [[f[1]['%s_Xrotation' % joint], f[1]['%s_Yrotation' % joint], f[1]['%s_Zrotation' % joint]] for
                ...
                #
                # print(euler[1])
                quat = [Rotation(f, 'euler2quat', from_deg=True).get_quat() for f in
                        euler]  # Convert the eulers to quat

                # Create the corresponding columns in the new DataFrame
                # print("---QUAT---")
                # print(quat)
                quat_df['%s_quatX' % joint] = pd.Series(data=[e[0] for e in quat], index=quat_df.index)
                quat_df['%s_quatY' % joint] = pd.Series(data=[e[1] for e in quat], index=quat_df.index)
                quat_df['%s_quatZ' % joint] = pd.Series(data=[e[2] for e in quat], index=quat_df.index)
                quat_df['%s_quatW' % joint] = pd.Series(data=[e[3] for e in quat], index=quat_df.index)
