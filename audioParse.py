...
xx, sr = librosa.load(audio_path, sr=48000)
# print(type(xx), type(sr))
# print(xx[1])
# mfcc = librosa.feature.mfcc(y=xx, sr=sr, n_mfcc=13)
# melspec = librosa.feature.melspectrogram(y=xx, sr=sr, hop_length=849)
#
# print(mfcc.shape,"hi\n")
# print(melspec.shape)

# librosa.time_to_frames(np.arange(0, 1, 0.1),sr=22050)


#
# <class 'numpy.ndarray'> <class 'int'>
# print(xx.shape, sr)
# test = librosa.get_duration(y=xx,sr=sr)
# print(test)
# (1102324,) 22050
x = librosa.resample(xx,sr,16000)
sr = 16000

librosa.output.write_wav(new_file, x, sr)
print(x.shape, 16000)
...
# new = np.zeros(shape=(len(time_stamps)-1,2,3))
# test = librosa.samples_to_frames(np.arange(0, 22050, 320))
...
for i in range(0,len(time_stamps)-1): # 0,1,2,3,4
    ...
    list[i].append(stft)
    # mfcc = librosa.feature.mfcc(y=audio_slice, sr=sr, hop_length=80, n_mfcc=13)
    # melspec = librosa.feature.melspectrogram(y=audio_slice, sr=sr)
    #
...
# print(melspec.shape,logspec.shape,stft.shape)
# print("before",mfcc.shape)
#
...
# stft2 = np.abs(librosa.stft(x, n_fft=160, hop_length=80, window=scipy.signal.hamming))
#
...
# file.close()
# for i in list:
#     print(i[0].shape,type(i[0]))
# # list1 = ['1','2','3','4']
# fileObject = open('audio/list.txt','w')
# for i in list:
